...
namespace USCG.Core.Telemetry
{
    ...
    public class TelemetryManager : MonoBehaviour
    {
        ...
        private Dictionary<MetricId, object> _metrics = new();
        ...
        private void Update()
        {
            if(!MetricsManager.IsTelemetryEnabled || !LevelManager.IsShortcutsEnabled) return;
        ...
        private void PrintMetricsImmediate(bool bResetMetrics = false, string levelName = null)
        {
            ...
            string filePath = $"{Application.persistentDataPath}/{Application.productName}-{levelName}-Metrics-{dateTime}.csv";
        ...
        public void ClearTimeSamples(MetricId timeSampleId)
        {
            if(_metrics == null || _metrics.Count == 0) return;
            var timeSamples = (TSampledMetric<DateTime>) _metrics[timeSampleId];
            timeSamples?.Reset();
        }
        ...
        public void RemoveTimeSampleFrom(MetricId timeSampleId, DateTime dateTime)
        {
            if(_metrics == null || _metrics.Count == 0) return;
            var timeSamples = (TSampledMetric<DateTime>) _metrics[timeSampleId];
            timeSamples?.RemoveSample(dateTime);
        }

        public void ExportMetricsToCsv(string levelName)
        {
            PrintMetricsImmediate(true, levelName);
        }
    }
}
